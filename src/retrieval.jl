...
function rephrase(
        ...
        verbose::Bool = true,
        model::String = PT.MODEL_CHAT,
        template::Symbol = :RAGQueryOptimizer,
...
"""
function rephrase(
        rephraser::HyDERephraser,
        question::AbstractString;
        verbose::Bool = true,
        model::String = PT.MODEL_CHAT,
        template::Symbol = :RAGQueryHyDE,
        cost_tracker = Threads.Atomic{Float64}(0.0),
        kwargs...
)
...
function find_closest(
    ...
    positions,
    scores = find_closest(finder, chunkdata(index),
...
function find_closest(
        ...
        query_tokens::AbstractVector{<:AbstractVector{<:AbstractString}} = Vector{Vector{String}}();
        top_k::Int = 100,
        kwargs...
)
    if isnothing(chunkdata(parent(index)))
...
function find_closest(
    ...
    for i in eachindex(all_indexes, finder.finders)
        positions_,
        scores_ = find_closest(finder[i], chunkdata(all_indexes[i]),
            query_emb, query_tokens;
...
"""
@inline function hamming_distance(
        mat::AbstractMatrix{T}, query::AbstractVector{T}
)::Vector{Int} where {T <: Integer}
...
"""
function find_closest(
        finder::BinaryCosineSimilarity,
        emb::AbstractMatrix{<:Bool},
        query_emb::AbstractVector{<:Real},
        query_tokens::AbstractVector{<:AbstractString} = String[];
        top_k::Int = 100,
    ...
    new_positions,
    scores = find_closest(CosineSimilarity(), @view(emb[:, positions]),
...
"""
function find_closest(
        finder::BitPackedCosineSimilarity,
        emb::AbstractMatrix{<:Integer},
        query_emb::AbstractVector{<:Real},
        query_tokens::AbstractVector{<:AbstractString} = String[];
        top_k::Int = 100,
        rescore_multiplier::Int = 4,
    ...
    new_positions,
    scores = find_closest(CosineSimilarity(), unpacked_emb,
        query_emb; top_k, minimum_similarity, kwargs...)
...
function find_tags(
        ...
        tag::Union{T, AbstractVector{<:T}};
        kwargs...
) where {T <: Union{AbstractString, Regex, Nothing}}
...
function find_tags(
        method::AnyTagFilter, index::AbstractChunkIndex, tags::Vector{T};
        kwargs...
...
"""
function find_tags(
        method::AllTagFilter, index::AbstractChunkIndex, tags_vec::Vector{T};
        kwargs...
) where {T <: Union{AbstractString, Regex}}
    isnothing(tags(index)) && CandidateChunks(; index_id = indexid(index))
    tag_idx = Int[]
...
function find_tags(
        method::Union{AnyTagFilter, AllTagFilter},
        index::AbstractMultiIndex,
        tag::Union{T, AbstractVector{<:T}};
        kwargs...
) where {T <: Union{AbstractString, Regex}}
...
function reciprocal_rank_fusion(mcc::MultiCandidateChunks; k::Int = 60)
    ...
    merged_positions,
    scores_dict = reciprocal_rank_fusion(
    ...
    return MultiCandidateChunks(
        fill(mcc.index_ids[1],
            length(merged_positions)),
        merged_positions,
...
function retrieve(index::AbstractChunkIndex, question::AbstractString; kwargs...)
