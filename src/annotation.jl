...
# Passthrough by default
function add_node_metadata!(
        annotater::AbstractAnnotater, root::AbstractAnnotatedNode; kwargs...
)
    root
end
...
function PromptingTools.pprint(
        node::AbstractAnnotatedNode;
        text_width::Int = displaysize(stdout)[2],
...
``
`"""
function trigram_support!(
        parent_node::AnnotatedNode,
        context_trigrams::AbstractVector,
        trigram_func::F1 = trigrams,
        token_transform::F2 = identity;
        skip_trigrams::Bool = false,
