...
function build_context(
    ...
    for (i, position) in enumerate(positions(candidates))
        ...
        max(
            1, position - chunks_window_margin[1]):min(end,
...
function answer!(
        answerer::AbstractAnswerer, index::AbstractDocumentIndex, result::AbstractRAGResult; kwargs...
)
...
"""
function airag(cfg::AbstractRAGConfig,
        index::AbstractDocumentIndex;
        question::AbstractString,
        verbose::Integer = 1,
